...
CLASS C_SVM
{
	...
	var string MILGreetings;				//Miliz grt Miliz oer Paladin
	var string PALGreetings;				//Paladin grt Paladin oder Magier
	var string Weather;						//So ein Mistwetter!
	
	// ------ B_Say_AttackReason ------
	var string IGetYouStill;				//Upset-NSC sieht Spieler whrned seiner Angry-Time wieder. Greift mit altem AttackReason an.
	var string DieEnemy;					//Feind wird angegriffen
	var string DieMonster;					//gefhrliches Monster wird angegriffen
	...
	var string DieStupidBeast;				//Torwachen schlagen ungefhliches Monster tot, das versucht durchs Tor zu gehen	
	...
	var string WillYouStopFighting;			//Wache beendet Kampf, greift Tter an
	
	// ------ B_Say_AttackEND ------
	var string KillEnemy;					//Finishing-Move
	var string EnemyKilled;					//NSC hat Feind gettet
	var string MonsterKilled;				//NSC hat gefhrliches Monster gettet
	...
	var string KillMurderer;				//Mrder finishen
	var string StupidBeastKilled;			//GateGuards tten neutrales Monster, das vorbeikam
	...
	var string ThereIsNoFightingHere;		//Wache hat Tter einer Schlgerei (derjenige, der angefangen hat) niedergeschlagen

	// ------ C_WantToFlee ------ 
	var string SpareMe;						//Tu mir nichts!
	var string RunAway;						//Scheie! Nichts wie weg!		

	// ------ C_WantToCallGuardsForFight ------
	var	string Alarm;						//Wache ruft Wache
	var string Guards;						//Harter Bursche ruft Wache
	var string Help;						//Brger ruft Hilfe
		
	// ------ B_AssessMurder ------
	var string GoodMonsterKill;				//Monster gettet - NSC findet das toll
	var string GoodKill;					//Mensch gettet - NSC findet das toll (war sein Feind)
	...
	var string YesGoOutOfHere;				//Spieler verlt verboten Raum artig wieder
	...
	var string OhMyGodItsAFight;			//NSC entdeckt einen Kampf, und findet das besttzend (Brger)
	...
	var string OhMyGodHesDown;				//Bestrzung ber brutales Niederschlagen
	...
	var string Awake;						//NSC wacht aus dem Zustand Sleep wieder auf (Aufwachen, rkeln)
	
	// ------ B_AssignAmbientNEWS ------
	var string ABS_COMMANDER;				//Ich hrte, du warst beim Kommandanten und hast die Sache wieder in Ordnung gebracht.
	var string ABS_MONASTERY;				//Ich hrte, du warst bei Vater Parlan und hast Bue getan.
	var string ABS_FARM;					//Ich hrte, du warst bei Lee und und hast die Sache wieder in Ordnung gebracht.
	var string ABS_GOOD;
	// ---------------------------------
	var string SHEEPKILLER_CRIME; 			//Einfach unsere Schafe zu schlachten! Mach, da du hier wegkommst!
	var string ATTACK_CRIME;				//Mit miesen Schlgern rede ich nicht!
	var string THEFT_CRIME;					//Geh mir aus den Augen, dreckiger Dieb!
	var string MURDER_CRIME;				//Mit gemeinen Mrdern rede ich nicht.
	// ---------------------------------
	var string PAL_CITY_CRIME; 				//Du bist eine Schande fr deinen Orden! Lord Hagen wird toben vor Wut!
	var string MIL_CITY_CRIME;				//Du bist eine Schande fr die Stadtwache! Lord Andre wird dir das sicher austreiben!
	...
	// ------ GOLD ------
	var string GOLD_1000;					//1000 Goldstcke.
	var string GOLD_950;					//950 Goldstcke.
	var string GOLD_900;					//900 Goldstcke.
	var string GOLD_850;					//850 Goldstcke.
	var string GOLD_800;					//800 Goldstcke.
	var string GOLD_750;					//750 Goldstcke.
	var string GOLD_700;					//700 Goldstcke.
	var string GOLD_650;					//650 Goldstcke.
	var string GOLD_600;					//600 GOldstcke.
	var string GOLD_550;					//550 Goldstcke.
	var string GOLD_500;					//500 Goldstcke.
	var string GOLD_450;					//450 Goldstcke.
	var string GOLD_400;					//400 Goldstcke.
	var string GOLD_350;					//350 Goldstcke.
	var string GOLD_300;					//300 Goldstcke.
	var string GOLD_250;					//250 Goldstcke.
	var string GOLD_200;					//200 Goldstcke.
	var string GOLD_150;					//150 Goldstcke.
	var string GOLD_100;					//100 Goldstcke.
	var string GOLD_90;						//90 Goldstcke.
	var string GOLD_80;						//80 Goldstcke.
	var string GOLD_70;						//70 Goldstcke.
	var string GOLD_60;						//60 Goldstcke.
	var string GOLD_50;						//50 Goldstcke.
	var string GOLD_40;						//40 Goldstcke.
	var string GOLD_30;						//30 Goldstcke.
	var string GOLD_20;						//20 Goldstcke.
	var string GOLD_10;						//10 Goldstcke.
	...
	VAR	string Smalltalk07;				// ...aber behalt's fr Dich, mu nicht gleich jeder wissen...
	VAR	string Smalltalk08;				// ...das passiert mir nicht nochmal...
	VAR	string Smalltalk09;				// ...an der Gechichte mu wohl doch was dran sein...
	VAR	string Smalltalk10;				// ...man mu eben aufpassen was man rumerzhlt...
	VAR	string Smalltalk11;				// ...solange ich damit nichts zu tun habe...
	VAR	string Smalltalk12;				// ...man darf auch nicht alles glauben, was man hrt...
	...
	VAR	string Smalltalk16;				// ...frher wre das ganz anders gelaufen...
	VAR	string Smalltalk17;				// ...gequatscht wird viel...
	VAR	string Smalltalk18;				// ...ich hr nicht mehr auf das Gefasel...
	...
	VAR	string Smalltalk23;				// ...ich dachte, das wre schon lange geklrt, aber das ist wohl nicht so...
	...
	VAR	string Smalltalk30;				// ...niemand darf gegen die gttliche Ordnung verstoen...
		
	// ------ Lehrer-Kommentare ------
	var string NoLearnNoPoints			;	//NSC-Lehrer verbietet Steigerung - keine Lernpunkte!
	var string NoLearnOverPersonalMAX	;	//NSC-Lehrer verbietet Steigerung BER sein persnliches Lehrer-Maximum
	...
	// ------ Kampf ------
	var	string Dead				;			// Tdlich verletzt
	...
	var string ADDON_WRONGARMOR;			//Sc hat unangemessene Rstung an				//Default
	var string ADDON_WRONGARMOR_SLD;		//Beide SLD/DJG und falsche Rstung
	var string ADDON_WRONGARMOR_MIL;		//Beide MIL/PAL	und falsche Rstung
	var string ADDON_WRONGARMOR_KDF;		//Beide KDF und falsche Rstung
	var string ADDON_NOARMOR_BDT;			//self ist BAndite und SC hat keine Rstung an.
...
// NSCs
// ----
// SVMs mssen immer "SVM_x" heissen, 
...
instance SVM_1 (C_SVM)
{
	...
	YouAskedForIt				=	"SVM_1_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	WiseMove					=	"SVM_1_WiseMove"					;//Es geht doch.
	// ------ ZS_Unconscious ------------------------------------------------------------------------------------------------
...
instance SVM_3 (C_SVM)
{
	...
	YouAskedForIt				=	"SVM_3_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	GetUpAndBeGone				=	"SVM_3_GetUpAndBeGone"				;//Und jetzt mach, dass du hier rauskommst!
	NeverEnterRoomAgain			=	"SVM_3_NeverEnterRoomAgain"		;//Und dass ich dich nie wieder da drin erwische!
	...
	// ------ ZS_ReactToWeapon ---------------------------------------------------------------------------------------------
	LookingForTroubleAgain		=	"SVM_3_LookingForTroubleAgain"	;//Hast du noch nicht genug?
	...
	OhMyHead					=	"SVM_3_OhMyHead"				;//(zu sich selbst) Oh, mein Schädel ...
	// ------ ZS_WatchFight -------------------------------------------------------------------------------------------------
	...
	Smalltalk21					= "SVM_3_Smalltalk21"				;//... warum erfahre ich das erst jetzt ...
	Smalltalk22					= "SVM_3_Smalltalk22"				;//... lass uns mal abwarten, was daraus wird ...
	...
	UNTERSTADT_2_OBERSTADT		= "SVM_3_UNTERSTADT_2_OBERSTADT"	;//Am südlichen Stadttor gibt es eine Treppe, die zum inneren Stadttor hochführt. Dort beginnt die Oberstadt.
...
instance SVM_4 (C_SVM)
{
	...
	Weather						= 	"SVM_4_Weather"						;//So ein Mistwetter!
	// ------ B_Say_AttackReason -----------------------------------------------------------------------------------------
	...
	GoodKill					= 	"SVM_4_GoodKill"				;//(ruft) Ja, mach das Schwein fertig!
	// ------ B_AssessTalk -----------------------------------------------------------------------------------------------
	...
	// ------ ZS_ObservePlayer ---------------------------------------------------------------------------------------------
	WhatsThisSupposedToBe		=	"SVM_4_WhatsThisSupposedToBe"	;//Was schleichst du da rum?
	...
	ISaidStopMagic				=	"SVM_4_ISaidStopMagic"			;//Willst du Schläge? Hör sofort damit auf!!!
	...
	OhMyHead					=	"SVM_4_OhMyHead"				;//(zu sich selbst) Oh, mein Schädel ...
	// ------ ZS_WatchFight -------------------------------------------------------------------------------------------------
	...
	UNTERSTADT_2_OBERSTADT		= "SVM_4_UNTERSTADT_2_OBERSTADT"	;//Am südlichen Stadttor gibt es eine Treppe, die zum inneren Stadttor hochführt. Dort beginnt die Oberstadt.
	...
	TEMPEL_2_MARKT				= "SVM_4_TEMPEL_2_MARKT"			;//Wenn du vorm Tempel stehst, geh links hoch und an der Stadtmauer entlang, dann kommst du zum Markt.
...
instance SVM_5 (C_SVM)
{
	...
	DieEnemy					=	"SVM_5_DieEnemy"					;//Jetzt kannst du was erleben!
	...
	YouAskedForIt				=	"SVM_5_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	RunAway						= 	"SVM_5_RunAway"						;//Scheiße! Nichts wie weg!
	// ------ C_WantToCallGuardsToFight --------------------------------------------------------------------------------------
	...
	// ------ ZS_ObservePlayer ---------------------------------------------------------------------------------------------
	WhatsThisSupposedToBe		=	"SVM_5_WhatsThisSupposedToBe"		;//Hey du! Was schleichst du da rum.
	...
	WiseMove					=	"SVM_5_WiseMove"					;//Es geht doch.
	// ------ ZS_Unconscious ------------------------------------------------------------------------------------------------
	...
	// -------------------------
	PAL_CITY_CRIME				= "SVM_5_PAL_CITY_CRIME"				;//Du bist eine Schande für deinen Orden! Lord Hagen wird toben vor Wut!
	...
	// ----------------------------------------------------------------------------------------------------------------------
	TOUGHGUY_ATTACKLOST			= "SVM_5_TOUGHGUY_ATTACKLOST"			;//Ist ja schon gut, du hast gewonnen. Was willst du?
	...
	Smalltalk04					= "SVM_5_Smalltalk04"					;//... als ob ich nicht schon genug Probleme hätte ...
	...
	Smalltalk22					= "SVM_5_Smalltalk22"					;//... lass uns mal abwarten, was daraus wird ...
	...
	UNTERSTADT_2_OBERSTADT		= "SVM_5_UNTERSTADT_2_OBERSTADT"	;//Am südlichen Stadttor gibt es eine Treppe, die zum inneren Stadttor hochführt. Dort beginnt die Oberstadt.
	...
	TEMPEL_2_MARKT				= "SVM_5_TEMPEL_2_MARKT"			;//Wenn du vorm Tempel stehst, geh links hoch und an der Stadtmauer entlang, dann kommst du zum Markt.
...
instance SVM_6 (C_SVM)
{
	...
	DirtyThief					=	"SVM_6_DirtyThief"					;//Na warte, du dreckiger Dieb!
	HandsOff					=	"SVM_6_HandsOff"					;//Finger weg da!
	...
	rumfummlerDown				=	"SVM_6_rumfummlerDown"				;//Lass in Zukunft die Finger von Sachen, an denen du nichts zu suchen hast!
	...
	UNTERSTADT_2_OBERSTADT		= "SVM_6_UNTERSTADT_2_OBERSTADT"	;//Am südlichen Stadttor gibt es eine Treppe, die zum inneren Stadttor hochführt. Dort beginnt die Oberstadt.
...
instance SVM_7 (C_SVM)
{
	...
	Weather						= 	"SVM_7_Weather"						;//So ein Mistwetter!
	// ------ B_Say_AttackReason -----------------------------------------------------------------------------------------
	...
	GoodKill					= 	"SVM_7_GoodKill"				;//(ruft) Ja, mach das Schwein fertig!
	// ------ B_AssessTalk -----------------------------------------------------------------------------------------------
	...
	// ------ ZS_ObservePlayer ---------------------------------------------------------------------------------------------
	WhatsThisSupposedToBe		=	"SVM_7_WhatsThisSupposedToBe"	;//Was schleichst du da rum?
	...
	ISaidWeaponDown				=	"SVM_7_ISaidWeaponDown"			;//Steck endlich die Scheißwaffe weg!
	...
	OhMyHead					=	"SVM_7_OhMyHead"				;//(zu sich selbst) Oh, mein Schädel ...
	// ------ ZS_WatchFight -------------------------------------------------------------------------------------------------
	...
	ATTACK_CRIME     = "SVM_7_ATTACK_CRIME"    ;//Du bist ein Schläger - hau ab!
 	...
 	// -------------------------
 	PAL_CITY_CRIME     = "SVM_7_PAL_CITY_CRIME"   ;//Von einem Paladin hätte ich mehr erwartet! Lord Hagen wird nicht sehr erfreut sein.
	...
	Smalltalk03					= "SVM_7_Smalltalk03"				;//... sag mir nicht, dass du das nicht gewusst hast ...
	Smalltalk04					= "SVM_7_Smalltalk04"				;//... man weiß gar nicht mehr, wem man noch glauben soll ...
	Smalltalk05					= "SVM_7_Smalltalk05"				;//... das ist doch alles nur Gerede ...
	Smalltalk06					= "SVM_7_Smalltalk06"				;//... meinst du, mir geht es besser ...
	Smalltalk07					= "SVM_7_Smalltalk07"				;//... ich wundere mich über gar nichts mehr ...
	...
	Smalltalk12					= "SVM_7_Smalltalk12"				;//... er hat auf die falschen Leute gehört ...
	...
	Smalltalk26					= "SVM_7_Smalltalk26"				;//... der weiß ja nicht mal, wie man ein Schwert hält ...
	...
	UNTERSTADT_2_OBERSTADT		= "SVM_7_UNTERSTADT_2_OBERSTADT"	;//Am südlichen Stadttor gibt es eine Treppe, die zum inneren Stadttor hochführt. Dort beginnt die Oberstadt.
...
instance SVM_8 (C_SVM)
{
	...
	// ------ B_Say_AttackReason -----------------------------------------------------------------------------------------
	IGetYouStill				= 	"SVM_8_IGetYouStill"				;//Aber jetzt krieg ich dich!
	...
	YouAskedForIt				=	"SVM_8_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	GetUpAndBeGone				=	"SVM_8_GetUpAndBeGone"				;//Und jetzt mach, dass du hier rauskommst!
	NeverEnterRoomAgain			=	"SVM_8_NeverEnterRoomAgain"		;//Und dass ich dich nie wieder da drin erwische!
	...
	// ------ ZS_ReactToWeapon ---------------------------------------------------------------------------------------------
	LookingForTroubleAgain		=	"SVM_8_LookingForTroubleAgain"	;//Hast du noch nicht genug?
	StopMagic					=	"SVM_8_StopMagic"				;//Hör auf mit dieser Magie-Scheiße!
	ISaidStopMagic				=	"SVM_8_ISaidStopMagic"			;//Willst du Schläge? Hör sofort damit auf!!!
	...
	OhMyHead					=	"SVM_8_OhMyHead"				;//(zu sich selbst) Oh, mein Schädel ...
	// ------ ZS_WatchFight -------------------------------------------------------------------------------------------------
	...
	// -------------------------
	PAL_CITY_CRIME				= "SVM_8_PAL_CITY_CRIME"			;//Du bist eine Schande für deinen Orden! Warte, bis Lord Hagen davon hört!
	...
	TOUGHGUY_PLAYERATTACK		= "SVM_8_TOUGHGUY_PLAYERATTACK"	;//Ich dachte, du wolltest dich mit mir anlegen. Hast du's dir anders überlegt? Willst doch lieber REDEN, was? (lacht)
	// ----------------------------------------------------------------------------------------------------------------------
	...
	Smalltalk03					= "SVM_8_Smalltalk03"				;//... sag mir nicht, dass du das nicht gewusst hast ...
	Smalltalk04					= "SVM_8_Smalltalk04"				;//... man weiß gar nicht mehr, wem man noch glauben soll ...
	...
	Smalltalk15					= "SVM_8_Smalltalk15"				;//... von selbst ist er da nicht drauf gekommen ...
	...
	TEMPEL_2_MARKT				= "SVM_8_TEMPEL_2_MARKT"			;//Wenn du vorm Tempel stehst, geh links hoch und an der Stadtmauer entlang, dann kommst du zum Markt.
	TEMPEL_2_GALGEN				= "SVM_8_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_8_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
...
instance SVM_9 (C_SVM)
{
	...
	// ------ B_Say_AttackReason -----------------------------------------------------------------------------------------
	IGetYouStill				= 	"SVM_9_IGetYouStill"				;//Aber jetzt krieg ich dich!
	...
	//MonsterKilled				=	"SVM_9_MonsterKilled"				;//Ich steh' auf diese ganze Scheie!
	...
	rumfummlerDown				=	"SVM_9_rumfummlerDown"				;//Lass in Zukunft die Finger von Sachen, an denen du nichts zu suchen hast!
	...
	ITakeYourWeapon				=	"SVM_9_ITakeYourWeapon"		;//Deine Waffe nehm ich mal mit.
	// ------ ZS_ReactToDamage ---------------------------------------------------------------------------------------------
	...
	Smalltalk14					= "SVM_9_Smalltalk14"				;//... sag nicht, dass hast du nicht gewusst ...
	...
	//ToughGuy (SLD/MIL/DJG)                                                                                                                        
	Smalltalk25					= "SVM_9_Smalltalk25"				;//... der ist gerannt, als ob Beliar selbst hinter ihm her wäre ...
	Smalltalk26					= "SVM_9_Smalltalk26"				;//... wir haben ihn natürlich doch noch gekriegt ...
	Smalltalk27					= "SVM_9_Smalltalk27"				;//... da muss man mal ganz kräftig dazwischen hauen ...
	//ProInnos (NOV/KDF/PAL)                                                                                                                        
	Smalltalk28					= "SVM_9_Smalltalk28"				;//... einige wissen gar nicht, wie sehr sie sich versündigen ...
	Smalltalk29					= "SVM_9_Smalltalk29"				;//... denn so ist Innos' Wille ...
	...
	MARKT_2_TEMPEL				= "SVM_9_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
	...
	KASERNE_2_MARKT				= "SVM_9_KASERNE_2_MARKT"			;//Geh einfach die Treppe am Haupteingang links runter, dann kommst du zum Marktplatz.
...
instance SVM_10 (C_SVM)
{
	...
	Weather						= 	"SVM_10_Weather"						;//So ein Mistwetter!
	// ------ B_Say_AttackReason -----------------------------------------------------------------------------------------
	...
	// ------ ZS_ObservePlayer ---------------------------------------------------------------------------------------------
	WhatsThisSupposedToBe		=	"SVM_10_WhatsThisSupposedToBe"	;//Was schleichst du da rum?
	...
	OhMyHead					=	"SVM_10_OhMyHead"				;//(zu sich selbst) Oh, mein Schädel ...
	// ------ ZS_WatchFight -------------------------------------------------------------------------------------------------
	...
	ATTACK_CRIME     = "SVM_10_ATTACK_CRIME"    ;//Du bist ein Schläger - hau ab!
 	...
 	// -------------------------
 	PAL_CITY_CRIME     = "SVM_10_PAL_CITY_CRIME"   ;//Von einem Paladin hätte ich mehr erwartet! Lord Hagen wird nicht sehr erfreut sein.
	...
	NoLearnYoureBetter			= "SVM_10_NoLearnYoureBetter"		;//Ich kann dir nichts mehr beibringen. Du bist schon zu gut.
	...
	// -----------------------
	WHERETO						= "SVM_10_WHERETO"					;//Wo willst du denn hin?
	// -----------------------
	...
	TEMPEL_2_GALGEN				= "SVM_10_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_10_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
	...
	GALGEN_2_KASERNE			= "SVM_10_GALGEN_2_KASERNE"			;//Die Kaserne ist das riesengroße Gebäude. Geh einfach die Treppen rauf.
...
instance SVM_11 (C_SVM)
{
	...
	DieEnemy					=	"SVM_11_DieEnemy"					;//Jetzt kannst du was erleben!
	...
	YouAskedForIt				=	"SVM_11_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	// ------ ZS_ReactToWeapon ---------------------------------------------------------------------------------------------
	LookingForTroubleAgain		=	"SVM_11_LookingForTroubleAgain"		;//(aggressiv) Hast du immer noch nicht genug?!
	...
	WiseMove					=	"SVM_11_WiseMove"					;//Es geht doch.
	// ------ ZS_Unconscious ------------------------------------------------------------------------------------------------
	...
	Smalltalk22					= "SVM_11_Smalltalk22"					;//... lass uns mal abwarten, was daraus wird ...
	...
	TEMPEL_2_GALGEN				= "SVM_11_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_11_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
...
instance SVM_12 (C_SVM)
{
	...
	// ------ B_Say_AttackReason -----------------------------------------------------------------------------------------
	IGetYouStill				= 	"SVM_12_IGetYouStill"				;//Aber jetzt krieg ich dich!
	...
	YouAskedForIt				=	"SVM_12_YouAskedForIt"				;//Ich hatte dich gewarnt!
	...
	rumfummlerDown				=	"SVM_12_rumfummlerDown"				;//Lass in Zukunft die Finger von Sachen, an denen du nichts zu suchen hast!
	...
	ThereIsNoFightingHere		=	"SVM_12_ThereIsNoFightingHere"			;//Hier wird nicht gekämpft, merk dir das!
	// ------ C_WantToFlee -------------------------------------------------------------------------------------------------
	...
	// ------ ZS_Attack ----------------------------------------------------------------------------------------------------
	RunCoward					=	"SVM_12_RunCoward"				;//(ruft laut) Ich krieg dich noch!
	// ------ ZS_ClearRoom -------------------------------------------------------------------------------------------------
	...
	ITakeYourWeapon				=	"SVM_12_ITakeYourWeapon"		;//Deine Waffe nehm ich mal mit.
	// ------ ZS_ReactToDamage ---------------------------------------------------------------------------------------------
	...
	Smalltalk07					= "SVM_12_Smalltalk07"				;//... ich könnte mir so etwas nicht erlauben ...
	...
	Smalltalk13					= "SVM_12_Smalltalk13"				;//... und glaub ja nicht, dass das alles ist ...
	...
	TEMPEL_2_GALGEN				= "SVM_12_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_12_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
	...
	GALGEN_2_KASERNE			= "SVM_12_GALGEN_2_KASERNE"			;//Die Kaserne ist das riesengroße Gebäude. Geh einfach die Treppen rauf.
...
instance SVM_13 (C_SVM)
{
	...
	// ------ B_Say_AttackReason -----------------------------------------------------------------------------------------
	IGetYouStill				= 	"SVM_13_IGetYouStill"				;//Jetzt krieg ich dich!
	...
	YouAskedForIt				=	"SVM_13_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	GetUpAndBeGone				=	"SVM_13_GetUpAndBeGone"				;//Und jetzt mach, dass du hier rauskommst!
	NeverEnterRoomAgain			=	"SVM_13_NeverEnterRoomAgain"		;//Und dass ich dich nie wieder da drin erwische!
	...
	// ------ ZS_Attack ----------------------------------------------------------------------------------------------------
	RunCoward					=	"SVM_13_RunCoward"				;//(ruft laut) Ja! Renn, so schnell du kannst!
	// ------ ZS_ClearRoom -------------------------------------------------------------------------------------------------
	...
	// ------ ZS_ReactToWeapon ---------------------------------------------------------------------------------------------
	LookingForTroubleAgain		=	"SVM_13_LookingForTroubleAgain"	;//Hast du noch nicht genug?
	...
	OhMyHead					=	"SVM_13_OhMyHead"				;//(zu sich selbst) Oh, mein Schädel ...
	// ------ ZS_WatchFight -------------------------------------------------------------------------------------------------
	...
	TEMPEL_2_GALGEN				= "SVM_13_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_13_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
...
instance SVM_14 (C_SVM)
{
	...
	DieEnemy					=	"SVM_14_DieEnemy"					;//Jetzt kannst du was erleben!
	...
	YouAskedForIt				=	"SVM_14_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	ITakeYourWeapon				=	"SVM_14_ITakeYourWeapon"				;//Deine Waffe nehm' ich mal lieber mit.
	// ------ ZS_ReactToDamage ---------------------------------------------------------------------------------------------
	WhatAreYouDoing				=	"SVM_14_WhatAreYouDoing"				;//(warnt) Hey! Pass auf!
	// ------ ZS_ReactToWeapon ---------------------------------------------------------------------------------------------
	LookingForTroubleAgain		=	"SVM_14_LookingForTroubleAgain"		;//(aggressiv) Hast du immer noch nicht genug?!
	...
	WiseMove					=	"SVM_14_WiseMove"					;//Es geht doch.
	// ------ ZS_Unconscious ------------------------------------------------------------------------------------------------
	...
	Smalltalk02					= "SVM_14_Smalltalk02"					;//... ich wär mir da nicht so sicher ...
	...
	Smalltalk14					= "SVM_14_Smalltalk14"					;//... sag nicht, dass hast du nicht gewusst ...
	Smalltalk15					= "SVM_14_Smalltalk15"					;//... das war doch klar ...
	Smalltalk16					= "SVM_14_Smalltalk16"					;//... wo hast du das denn schon wieder her ...
	Smalltalk17					= "SVM_14_Smalltalk17"					;//... das kann ja nicht ewig so weitergehen ...
	Smalltalk18					= "SVM_14_Smalltalk18"					;//... ich hab da meine eigene Meinung ...
	...
	Smalltalk27					= "SVM_14_Smalltalk27"					;//... da muss man mal ganz kräftig dazwischen hauen ...
	//ProInnos (NOV/KDF/PAL)                                                                                                                           
	Smalltalk28					= "SVM_14_Smalltalk28"					;//... einige wissen gar nicht, wie sehr sie sich versündigen ...
	Smalltalk29					= "SVM_14_Smalltalk29"					;//... denn so ist Innos' Wille ...
	...
	TEMPEL_2_GALGEN				= "SVM_14_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_14_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
...
INSTANCE SVM_15	(C_SVM)		//	PLAYER	trocken,ernst
{
	...
	TRADE_3					=	"SVM_15_Addon_TRADE_3"					;//Zeig mir deine Ware
...
instance SVM_16 (C_SVM)
{
	...
	DieEnemy					=	"SVM_16_DieEnemy"					;//Jetzt kannst du was erleben!
	DieMonster					=	"SVM_16_DieMonster"					;//Da ist wieder eins von diesen Drecksviechern!
	...
	YouAskedForIt				=	"SVM_16_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	EnemyKilled					=	"SVM_16_EnemyKilled"				;//Das war's für dich, Penner.
	...
	NeverEnterRoomAgain			=	"SVM_16_NeverEnterRoomAgain"			;//Und dass ich dich nie wieder da drin erwische!
	ThereIsNoFightingHere		=	"SVM_16_ThereIsNoFightingHere"		;//Hier wird nicht gekämpft, klar!? Lass dir das eine Lehre sein!
	// ------ C_WantToFlee -------------------------------------------------------------------------------------------------
	SpareMe						=	"SVM_16_SpareMe"						;//Tu mir nichts!
	RunAway						= 	"SVM_16_RunAway"						;//Ich hau ab!
	// ------ C_WantToCallGuardsToFight --------------------------------------------------------------------------------------
	...
	YesGoOutOfHere				= 	"SVM_16_YesGoOutOfHere"			;//Ja, mach, dass du wegkommst!
	// ------ ZS_ObservePlayer ---------------------------------------------------------------------------------------------
	...
	// ------ ZS_ReactToWeapon ---------------------------------------------------------------------------------------------
	LookingForTroubleAgain		=	"SVM_16_LookingForTroubleAgain"	;//Hast du noch nicht genug?
	...
	FARM_CRIME      			= "SVM_16_FARM_CRIME"    			;//Lee wird erfahren, was du angestellt hast - und er wird dich bestrafen.
 	OC_CRIME       				= "SVM_16_OC_CRIME"     			;//Kommandant Garond wird dich zur Rechenschaft ziehen!
	// ----------------------------------------------------------------------------------------------------------------------
	...
	TEMPEL_2_GALGEN				= "SVM_16_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_16_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
...
instance SVM_17 (C_SVM)
{
	...
	DieEnemy					=	"SVM_17_DieEnemy"					;//Jetzt kannst du was erleben!
	...
	YouAskedForIt				=	"SVM_17_YouAskedForIt"				;//Das hast du dir selbst zuzuschreiben!
	...
	EnemyKilled					=	"SVM_17_EnemyKilled"				;//Das war's für dich, Penner.
	...
	RunAway						= 	"SVM_17_RunAway"						;//Ich hau ab!
	// ------ C_WantToCallGuardsToFight --------------------------------------------------------------------------------------
	...
	// ------ ZS_ClearRoom -------------------------------------------------------------------------------------------------
	GetOutOfHere				=	"SVM_17_GetOutOfHere"			;//Raus hier!
	...
	// ------ ZS_ReactToWeapon ---------------------------------------------------------------------------------------------
	LookingForTroubleAgain		=	"SVM_17_LookingForTroubleAgain"	;//Hast du noch nicht genug?
	...
	FARM_CRIME      			= "SVM_17_FARM_CRIME"    			;//Lee wird erfahren, was du angestellt hast - und er wird dich bestrafen.
 	OC_CRIME       				= "SVM_17_OC_CRIME"     			;//Kommandant Garond wird dich zur Rechenschaft ziehen!
	// ----------------------------------------------------------------------------------------------------------------------
	...
	TEMPEL_2_GALGEN				= "SVM_17_TEMPEL_2_GALGEN"			;//Wenn du vom Tempel aus gesehen links an der Kneipe vorbei gehst, kommst du zum Galgenplatz.
	MARKT_2_TEMPEL				= "SVM_17_MARKT_2_TEMPEL"			;//Wenn du vom Marktplatz aus an der hohen Stadtmauer entlang gehst, kommst du zum Tempel.
